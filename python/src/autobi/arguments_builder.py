...
class ArgumentBuilder:
    ...
    def with_input_TextGrid(self, textgrid: str) -> Self:
        self._object.withInputTextGrid(textgrid)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_input_wav(self, wav: str) -> Self:
        self._object.withInputWav(wav)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_output_file(self, outfile: str) -> Self:
        self._object.withOutputFile(outfile)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_output_arff_file(self, output_arff_file: str) -> Self:
        self._object.withOutputArffFile(output_arff_file)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_pitch_accent_detector(self, detector: str) -> Self:
        self._object.withPitchAccentDetector(detector)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_pitch_accent_classifier(self, classifier: str) -> Self:
        self._object.withPitchAccentClassifier(classifier)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_intonal_phrase_boundary_detector(self, detector: str) -> Self:
        self._object.withIntonalPhraseBoundaryDetector(detector)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_intermediate_phrase_boundary_detector(self, detector: str) -> Self:
        self._object.withIntermediatePhraseBoundaryDetector(detector)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_boundary_tone_classifier(self, classifier: str) -> Self:
        self._object.withBoundaryToneClassifier(classifier)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_phrase_accent_classifier(self, classifier: str) -> Self:
        self._object.withPhraseAccentClassifier(classifier)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_log4j_configfile(self, configfile: str) -> Self:
        self._object.withLog4jConfigFile(configfile)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_cprom_file(self, configfile: str) -> Self:
        self._object.withCpromFile(configfile)
        return self

    @takes(Path, converter=to_resolved_path_str)
    def with_rhapsodie_file(self, configfile: str) -> Self:
        self._object.withRhapsodieFile(configfile)
        return self

    def with_silence_regex(self, name: str) -> Self:
        self._object.withSilenceRegex(name)
        return self

    def with_words_tiername(self, name: str) -> Self:
        self._object.withWordsTierName(name)
        return self

    def with_breaks_tiername(self, name: str) -> Self:
        self._object.withBreaksTierName(name)
        return self

    def with_charset(self, new_charset: str) -> Self:
        self._object.withCharset(new_charset)
        return self

    def with_end_idx(self, idx: str) -> Self:
        self._object.withEndIDX(idx)
        return self

    def with_ortho_idx(self, idx: str) -> Self:
        self._object.withOrthoIDX(idx)
        return self

    def with_start_idx(self, idx: str) -> Self:
        self._object.withStartIDX(idx)
        return self

    @takes(float, converter=to_java_float)
    def with_silence_threshold(self, threshold: str) -> Self:
        self._object.withSilenceThreshold(threshold)
        return self

    @takes(bool, converter=to_java_bool)
    def with_distributions(self, distributions: str) -> Self:
