 ...
 */
public class SpectrumPADFeatureExtractorTest {
  private SpectrumPADFeatureExtractor fe;
  ...
  public void setUp() {
    ...
    fe = new SpectrumPADFeatureExtractor(0, 1, classifier, fs);
  }
  ...
  public void testExtractFeatureExtractsFeaturesCorrectly() {
    ...
    try {
      ...
      for (Word word : words) {
        ...
        assertEquals("ACCENTED", word.getAttribute("nominal_bark_0_1__prediction"));
        assertEquals(0.6666, (Double) word.getAttribute("bark_0_1__prediction_confidence"), 0.0001);
        assertEquals(
            0.6666, (Double) word.getAttribute("bark_0_1__prediction_confidence_accented"), 0.0001);
      }
    } catch (FeatureExtractorException e) {
      fail(e.getMessage());
    }
