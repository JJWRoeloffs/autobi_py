...
public class AuToBI {
	 ...
	 * @throws AuToBIException           If there are other problems
	 */
	public void extractFeatures(Set<String> features, FeatureSet fs) throws FeatureExtractorException, AuToBIException {
	...
	public void initializeFeatureRegistry(FeatureSet fs)
			throws AuToBIException, IllegalAccessException, InvocationTargetException, InstantiationException {
		Stack<String> features = new Stack<String>();
		if (fs.getClassAttribute() != null) {
			features.add(fs.getClassAttribute());
		}
	...
	protected String evaluateTaskPerformance(String task, FeatureSet fs) throws AuToBIException {
		if (tasks.containsKey(task)) {
			...
			return ClassifierUtils.evaluateClassification(autobi_task.getHypFeature(), autobi_task.getTrueFeature(),
					fs);
		} else {
	...
	public void generatePredictionsWithConfidenceScores(String task, FeatureSet fs) throws AuToBIException {
		if (tasks.containsKey(task)) {
			...
			ClassifierUtils.generatePredictionsWithConfidenceScores(autobi_task.getClassifier(),
					autobi_task.getHypFeature(), autobi_task.getConfFeature(), autobi_task.getDefaultValue(), fs);
		} else {
			throw new AuToBIException("Task, " + task
					+ ", is unavailable. Either no classifier has been set, or there is some other problem");
		}
		...
		for (Word w : words) {
			...
			if (w.hasAttribute("hyp_phrase_boundary")) {
				if (!w.getAttribute("hyp_phrase_boundary").equals("NONBOUNDARY")) {
					...
					text_grid.append("mark = \"").append(w.getAttribute("hyp_phrase_boundary").toString())
							.append("\"\n");
				}
			}
		}
		...
		for (int i = 0; i < words.size(); ++i) {
			...
			if (getBooleanParameter("distributions", false)) {
				if (w.hasAttribute(tasks.get("intonational_phrase_boundary_detection").getDistFeature())) {
					text = w.getAttribute(tasks.get("intonational_phrase_boundary_detection").getDistFeature())
							.toString();
				}
				if (w.hasAttribute(tasks.get("intermediate_phrase_boundary_detection").getDistFeature())) {
					text = w.getAttribute(tasks.get("intermediate_phrase_boundary_detection").getDistFeature())
							.toString();
				}
	...
	public void registerFeatureExtractorMonikers(String package_name) {
		...
		Set<Class<? extends FeatureExtractor>> fes = reflections
				.getSubTypesOf(edu.cuny.qc.speech.AuToBI.core.FeatureExtractor.class);
		...
		for (int low = 0; low <= 19; ++low) {
			for (int high = low + 1; high <= 20; ++high) {
				...
				for (String feature_prefix : new String[] { "bark_tilt", "bark" }) {
					...
					for (int prev = 0; prev < 3; prev++) {
						for (int foll = 0; foll < 3; foll++) {
							registerFeatureExtractor(new TemporalContextNormalizedFeatureExtractor(feature_name,
									400 * prev, 400 * foll));
						}
					}
				}
	...
	private void registerPitchAccentCollectionFeatureExtractors() throws FeatureExtractorException {
		try {
			...
			if (o instanceof PitchAccentDetectionClassifierCollection) {
				...
				throw new FeatureExtractorException("Object read from -spectral_pitch_accent_detector_collection="
						+ pad_filename + " is not a valid PitchAccentDetectionClassifierCollection");
			}
			...
			for (int low = 0; low < high_bark; ++low) {
				for (int high = low + 1; high <= high_bark; ++high) {
					...
					registerFeatureExtractor(new CorrectionSpectrumPADFeatureExtractor(low, high,
							pacc.getCorrectionClassifier(low, high), new CorrectionSpectrumPADFeatureSet(low, high)));
				}
			}
	...
	public void run() {
		try {
			...
			if (word_reader == null) {
				AuToBIUtils
						.error("Unable to create wordreader for file: " + filename + "\n\tCheck the file extension.");
				return;
			}
			...
			for (AuToBITask task : tasks.values()) {
				...
				if (classifier == null) {
					AuToBIUtils.error(
							"Classifier for task, " + task.getTrueFeature() + ", is unavailable. Check the filename.");
					continue;
				}
