 ...
 * TimeValuePairs. <p/> This operation requires a ContextDesc object to describe the context. <p/>
 ...
 * <p/>
 * If the attribute is a list of TimeValuePairs, zscore normalization of the min, max and mean
 * within the current word based on the context are calculated. <p/>
 * TODO: This FeatureExtractor should be divided into two feature extractors, one for contour
 * features and another for Doubles.
 *
...
public class ContextNormalizedFeatureExtractor extends FeatureExtractor {
   ...
   */
  public ContextNormalizedFeatureExtractor(String attribute_name, String context_desc)
  ...
  /**
   * Extracts features from a list of regions with a distinct list of regions to describe the
   * context. <p/> This version of the function decouples the extraction of the value to be
   * normalized from the normalization context. This is helpful when performing context
   * normalization of subregions.
   *
  ...
  private void extractContextNormAttributes(Region r, Double start, Double end)
    ...
    if (r.getAttribute(attribute_name) instanceof Double value) {
      ...
      if ((max - min) > EPSILON) {
        r.setAttribute(
            "rNormWordContext[" + context_feature_stem + "]", (value - min) / (max - min));
      }
