...
/**
 * SubregionFeatureExtractor constructs "subregions" aligned with the end of the supplied regions
 * and preceding the ending boundary by a specified amount of time. <p/> While it is intended that
 * the subregions are completely contained within the initial region, there is no enforcement of
 * this policy.  Thus subregions can in fact be longer than the initial region.  E.g. if the initial
 * region is 100ms and a 200ms region is requested the "subregion" is longer than the initial
 * region.
 */
