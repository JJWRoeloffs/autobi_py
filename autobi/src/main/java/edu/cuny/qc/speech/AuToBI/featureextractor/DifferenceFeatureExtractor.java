...
/**
 * Calculates the difference of a previously extracted numeric feature on each region and the
 * following region. <p/> Note: when using this feature extractor, it is assumed that the list of
 * regions are properly ordered.
 */
    ...
    for (int i = 0; i < regions.size() - 1; ++i) {
      ...
      if (r.hasAttribute(feature) && next_r.hasAttribute(feature)) {
        if (!(r.getAttribute(feature) instanceof Number)) {
          throw new FeatureExtractorException("Cannot calculate difference. Feature, " + feature
              + ", is not a Number on region, " + r);
        }
        if (!(next_r.getAttribute(feature) instanceof Number)) {
          throw new FeatureExtractorException("Cannot calculate difference. Feature, " + feature
              + ", is not a Number on region, " + next_r);
        }

        Number value = ((Number) next_r.getAttribute(feature)).doubleValue()
            - ((Number) r.getAttribute(feature)).doubleValue();
