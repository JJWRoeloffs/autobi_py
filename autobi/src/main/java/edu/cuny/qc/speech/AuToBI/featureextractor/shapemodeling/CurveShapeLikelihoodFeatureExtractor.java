  ...
  public void extractFeatures(List regions) throws FeatureExtractorException {
    for (Region r : (List<Region>) regions) {
      if (r.hasAttribute("risingCurve[" + feature + "]")
          ...
          && r.hasAttribute("valleyCurve[" + feature + "]")) {
        CurveShape rising = (CurveShape) r.getAttribute("risingCurve[" + feature + "]");
        CurveShape falling = (CurveShape) r.getAttribute("fallingCurve[" + feature + "]");
        CurveShape peak = (CurveShape) r.getAttribute("peakCurve[" + feature + "]");
        CurveShape valley = (CurveShape) r.getAttribute("valleyCurve[" + feature + "]");

        double maxrmse =
            Math.max(rising.rmse, Math.max(falling.rmse, Math.max(peak.rmse, valley.rmse)));
        ...
        try {
          ...
          // This exception is thrown when the distribution is even -- therefore all rmse values are
          // equal to the max. Since this happens so often, we'll set this to be even.
          d.put("rising", 0.25);
          d.put("falling", 0.25);
          d.put("peak", 0.25);
          d.put("valley", 0.25);
