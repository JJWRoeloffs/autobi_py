...
/**
 * A feature extractor to assign speaker normalization parameters (SNPs) to each word for acoustic
...
public class SNPAssignmentFeatureExtractor extends FeatureExtractor {
   ...
   * <p/>
   * This has two operating modes.  In one mode, the speaker normalization parameters are assigned
   * to regions without regard for the speaker id feature of the regions. <p/> This happens when a
   * new corpus is read and the Reader does not know how to correctly set the speaker_id parameter.
   * In this case, a single snp_file should be delivered to this constructor, and the contained
   * parameters will be assigned to all regions during feature extraction. <p/> The second operating
   * mode loads a set of speaker normalization paramaters and assigns them to regions by matching
   * the stored speaker_id parameter on the parameterization files with those set by the Reader when
   ...
   * @throws edu.cuny.qc.speech.AuToBI.core.AuToBIException If speaker id feature is null and there
   *     are more snp
   *                                                        files set
   */
  public SNPAssignmentFeatureExtractor(String speaker_id_feature, List<String> snp_files)
    ...
    if (speaker_id_feature == null) {
      this.default_snp =
          SpeakerNormalizationParameterGenerator.readSerializedParameters(snp_files.get(0));
      this.snp_map = null;
    } else {
  ...
  private void loadSNPList(List<String> files) {
    for (String filename : files) {
      SpeakerNormalizationParameter snp =
          SpeakerNormalizationParameterGenerator.readSerializedParameters(filename);
   ...
   * <p/>
   * If there is a speaker_id_feature set, then parameters are assigned based on the speaker_id
   * feature and the speaker_id feature associated with the speaker normalization parameters at
   * generation. <p/> If there is no speaker_id feature set, all regions will be assigned the same
   * parameter, loaded during construction.
   *
    ...
    for (Region r : (List<Region>) regions) {
      if (speaker_id_feature != null) {
        if (snp_map.containsKey(r.getAttribute(speaker_id_feature).toString())) {
          r.setAttribute(
              destination_feature, snp_map.get(r.getAttribute(speaker_id_feature).toString()));
        }
      } else {
