...
public class SubregionContourExtractor extends FeatureExtractor {
   ...
   * <p/>
   * Extracts a new subcontour feature called "feature_name"_"subregion_feature" which is a
   * subcontour of the feature name extracted from the region defined by subregion.
   *
  ...
  public void extractFeatures(List<Region> regions) throws FeatureExtractorException {
    for (Region r : regions) {
      ...
      if (c != null && r.hasAttribute(subregion_feature)) {
        ...
        try {
          Contour subcontour =
              ContourUtils.getSubContour(c, subregion.getStart(), subregion.getEnd());
          r.setAttribute(
              moniker + "[" + contour_feature + "," + subregion_feature + "]", subcontour);
