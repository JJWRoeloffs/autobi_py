...
/**
 * ToBIUtils is a utility class to store static methods used in the processing and interpretation of
 * ToBI annotations, specifically breaks and tones. <p/> Note: It may be preferred to fold many of
 * these functions into the FeatureExtractor framework.  Currently they remain as static methods,
 * some of which contain FeatureExtractor wrappers.  This is a likely future modification.
 */
public class ToBIUtils {
  /**
   * Constructs an intermediate phrase boundary annotation, either NONBOUNDARY,
   * INTERMEDIATE_BOUNDARY or INTONATIONAL_BOUNDARY based on analysis of associated breaks
   * information
   *
  ...
  public static void setPhraseAccentBoundaryTone(List<Word> data_points, String attribute_name) {
    for (Word word : data_points) {
      String label = "NOTONE";
      if (word.getBoundaryTone() != null)
        label = word.getPhraseAccent() + word.getBoundaryTone().replace('%', 'x');
      word.setAttribute(attribute_name, label);
    }
  }
  ...
  public static void setPhraseAccent(List<Word> words, String attribute_name) {
    for (Word word : words) {
      String label = "NOTONE";
      if (word.getPhraseAccent() != null)
        label = word.getPhraseAccent();
      word.setAttribute(attribute_name, label);
    }
  }
   ...
   * <p/>
   * Assumes that the breaks have already been assigned.  If some phrase ending tones have been
   * assigned, these will not be overwritten. However, inappropriate tones will be deleted -- for
   * example, if the break is 1, and there is a phrase accent, it will be deleted.
   *
   * @param words the set of words
   */
   ...
   * <p/>
   * Checks that 1) every intermediate phrase has a phrase accent. 2) every intermediate phrase
   * contains an accented word. 3) every intonational phrase has a boundary tone
   *
   * @param words The words to check
   * @throws edu.cuny.qc.speech.AuToBI.core.AuToBIException when word annotations to not match the
   *     ToBI standard
   */
  public static void checkToBIAnnotations(List<Word> words) throws AuToBIException {
    ...
    for (Word word : words) {
      ...
      if (word.isIntermediatePhraseFinal() && !word.hasPhraseAccent()) {
        word.setPhraseAccent("X-?");
        AuToBIUtils.warn("Word, " + word + ", has an intermediate phrase ending break, "
            + word.getBreakAfter() + ", but no phrase accent");
      }
      if (word.isIntonationalPhraseFinal() && !word.hasBoundaryTone()) {
        word.setBoundaryTone("X%?");
        AuToBIUtils.warn("Word, " + word + ", has an intonational phrase ending break, "
            + word.getBreakAfter() + ", but no boundary tone");
      }

      if (!word.isIntermediatePhraseFinal() && word.hasPhraseAccent()) {
        throw new AuToBIException("Word, " + word + ", has a phrase accent, "
            + word.getPhraseAccent() + ", but no intermediate phrase ending break, "
            + word.getBreakAfter());
      }
      if (!word.isIntonationalPhraseFinal() && word.hasBoundaryTone()) {
        throw new AuToBIException("Word, " + word + ", has a boundary tone, "
            + word.getBoundaryTone() + ", but no intonational phrase ending break, "
            + word.getBreakAfter());
      }
      ...
      if (end_of_phrase && !has_accented_word) {
        AuToBIUtils.warn("The intermediate phrase that started at the word, " + starting_word
            + ", and ended at the word, " + word + ", contains no pitch accent bearing word.");
      }
    }
  }
   ...
   * <p/>
   * These are returned as an array containing 3 strings for the pitch accent, phrase acent, and
   * boutary tone present int helabel
   *
  ...
  public static String[] parseToneString(String label) {
    ...
    Pattern accent_pattern = Pattern.compile(
  ...
  public static String getPhraseAccent(String label) {
    if (label.startsWith("L-") || label.startsWith("H-") || label.startsWith("!H-")
        || label.startsWith("X-?")) {
  ...
  public static String getBoundaryTone(String label) {
    if (label.endsWith("H%") || label.endsWith("L%") || label.endsWith("!H%")
        || label.endsWith("X%?")) {
