...
/**
 * SimpleWordReader reads orthographic information from a text file containing start times, end
 * times, and the lexical item. <p/> A user may specify the order of the three fields, orthography,
 * start time and end time.  The time is assumed to be in seconds. Default Format: <p/>
...
public class SimpleWordReader extends AuToBIWordReader {
   ...
   */
  public SimpleWordReader(
      String filename, String charset_name, int ortho_idx, int start_idx, int end_idx) {
  ...
  public List<Word> readWords() throws IOException, AuToBIException {
    ...
    while ((line = file_reader.readLine()) != null) {
      ...
      if (end < start) {
        throw new AuToBIException(
            "End time before start time on line " + file_reader.getLineNumber() + " - " + line);
      }
