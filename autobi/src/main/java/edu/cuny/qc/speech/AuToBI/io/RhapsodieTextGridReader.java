...
public class RhapsodieTextGridReader extends TextGridReader {
  ...
  public RhapsodieTextGridReader(
      String filename, String words_tier, String tone_tier, String charset) {
   ...
   * Typical Usage:
   * <p/>
   * TextGridReader reader = new TextGridReader(filename) List<Words> data_points =
   * reader.readWords();
   *
  ...
  public List<Word> readWords() throws IOException, AuToBIException {
    ...
    // Merge word final schwas with previous syllables (make sure to take the union of the tone
    // annotations expid: SchwaCollapse
  ...
  /**
   * When identifying the final syllable in a word, ignore any final schwa, considering the
   * penultimate syllable to be 'final'.
   *
  ...
  private void ignoreFinalSchwas(List<Word> syllables) {
    ...
    while (si < syllables.size()) {
      ...
      if (s.getLabel().endsWith("@") && s.getAttribute("word_final").equals("true")) {
        ...
        s_prev.setBoundaryTone(
            s_prev.getBoundaryTone().replace("%", "") + s.getBoundaryTone().replace("%", "") + "%");
  ...
  /**
   * Merge syllables that end a word with a schwa to the previous syllable.  These are prosodically
   * neutral, serving mostly to displace the boundary of the word from prosodic boundary.
   *
   ...
   */
  private void readTiersWithReader(AuToBIFileReader file_reader)
   ...
   */
  protected void constructToBILikeAnnotations(List<Word> words, List<Region> tones)
    ...
    for (Word word : words) {
      ...
      if (tone.contains("i")) {
        ...
        if (tone.contains("*")) {
          ...
          if (!tone.contains(
                  "%")) { // displaced phrase ending. Make sure that the ending boundary is found.
            word.setBoundaryTone(tone + "%");
            String next_tone = "";
            int i = j + 1;
