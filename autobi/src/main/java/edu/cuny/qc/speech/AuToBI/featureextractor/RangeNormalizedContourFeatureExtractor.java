...
/**
 * NormalizedContourFeatureExtractor normalizes a contour represented by a list of time value pairs
 * and aligns the resulting normalized contour to the supplied regions. <p/> The normalization is
 * performed using range normalization ((x-min) / (max-min)) based on a supplied
...
public class RangeNormalizedContourFeatureExtractor extends FeatureExtractor {
  ...
  /**
   * Performs z-score normalization on the specified contour, storing the result in a new list of
   * TimeValuePairs
   *
   * @param regions The regions to extract features from.
   * @throws edu.cuny.qc.speech.AuToBI.featureextractor.FeatureExtractorException if the
      ...
      if (r.hasAttribute(feature_name) && r.hasAttribute(norm_feature)) {
        ...
        if (cache.containsKey(c)) {
          ...
          SpeakerNormalizationParameter norm_params =
              (SpeakerNormalizationParameter) r.getAttribute(norm_feature);
          if (norm_params.canNormalize(feature_name)) {
            Contour norm_contour = ContourUtils.rangeNormalizeContour(c, norm_params, feature_name);
