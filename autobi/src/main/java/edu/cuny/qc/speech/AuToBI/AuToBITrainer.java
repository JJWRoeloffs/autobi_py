...
/**
 * A Class to handle the training of autobi models, based on a set of training files, a FeatureSet
 * describing the required features, and an AuToBIClassifier to train. <p/> This class is used to
 * drive the training of all AuToBI classifiers.  AuToBITask objects store the information required
 * to train classifiers, and are used to maintain consistency around AuToBI.     This
 * parameterization allows this class to treat all tasks identically. <p/> The only difference
 * between AuToBI tasks is that the prosodic event classification tasks are trained only on data
...
public class AuToBITrainer {
   ...
   * <p/>
   * Reads a set of filenames. Extract a set of features described by a FeatureSet object. Trains a
   * classifier.  An empty classifier to be trained should be passed in.  This allows a user to
   ...
   */
  public void trainClassifier(Collection<FormattedFile> filenames, FeatureSet fs,
      AuToBIClassifier classifier) throws Exception {
    if (filenames.size() == 0) {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      try {
        filenames = AuToBIReaderUtils.globFormattedFiles(
            autobi.getParameter("cprom_filenames"), FormattedFile.Format.CPROM);
      } catch (AuToBIException e1) {
        try {
          filenames = AuToBIReaderUtils.globFormattedFiles(
              autobi.getParameter("rhapsodie_filenames"), FormattedFile.Format.RHAPSODIE);
        } catch (AuToBIException e2) {
          AuToBIUtils.error(
              "No training files specified with -training_filenames, -cprom_filenames or -rhapsodie_filenames");
          return;
        }
      }
    }
      ...
      try {
        ...
        if (task_label.equals("phrase_accent_classification")) {
          autobi.getParameters().setParameter(
              "attribute_omit", autobi.getTrueFeature("phrase_accent_classification") + ":NOTONE");
        } else if (task_label.equals("pitch_accent_classification")) {
          autobi.getParameters().setParameter(
              "attribute_omit", autobi.getTrueFeature("pitch_accent_classification") + ":NOACCENT");
